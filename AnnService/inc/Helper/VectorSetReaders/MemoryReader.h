...
namespace SPTAG
{
    namespace Helper
    {
        ...
        public:
            ...
            virtual std::shared_ptr<VectorSet> GetVectorSetRatio(float readRatio = 1, SizeType start = 0, SizeType end = -1) const 
            {
                if (end < 0 || end > m_vectors->Count()) end = m_vectors->Count() * readRatio;
                return std::shared_ptr<VectorSet>(new BasicVectorSet(ByteArray((std::uint8_t*)(m_vectors->GetVector(start)), (end - start) * m_vectors->PerVectorDataSize(), false),
                    m_vectors->GetValueType(),
                    m_vectors->Dimension(),
                    end - start));
