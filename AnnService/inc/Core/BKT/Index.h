...
#undef DefineBKTParameter
            ...
            ErrorCode SearchIndexWithGPU(QueryResult &p_query, std::shared_ptr<VectorSet> PQvectorSet, std::shared_ptr<std::uint64_t> p_vectorTranslateMap, bool p_searchDeleted = false) const;
        ...
        private:
            ...
            void SearchIndex(COMMON::QueryResultSet<T>& p_query, COMMON::WorkSpace& p_space) const;

            // void SearchIndexWithGPU(COMMON::QueryResultSet<T> &p_query, std::shared_ptr<VectorSet> PQvectorSet, COMMON::WorkSpace &p_space, std::shared_ptr<std::uint64_t> p_vectorTranslateMap) const;
