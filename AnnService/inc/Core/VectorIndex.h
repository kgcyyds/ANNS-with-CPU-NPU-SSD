...
public:
    ...
    // virtual ErrorCode SearchIndexWithGPU(QueryResult& p_results, std::shared_ptr<VectorSet> PQvectorSet, std::shared_ptr<std::uint64_t> p_vectorTranslateMap, bool p_searchDeleted = false) const
    // {
    ...
    virtual void ApproximateFusion(std::shared_ptr<VectorSet>& fullVectors, std::unordered_set<SizeType>& exceptIDS, int candidateNum, Edge* selections, int replicaCount, int numThreads, int numTrees, int leafSize, float RNGFactor, int numGPUs, int postingSizeLimit);
