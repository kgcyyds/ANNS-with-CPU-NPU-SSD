...
namespace SPTAG
{
    ...
    namespace SPANN
    {
        ...
        private:
            ...
            std::vector<int> m_vectorMapPosting;
            size_t m_pageSize = sysconf(_SC_PAGESIZE);
        ...
        public:
            ...
            inline std::shared_ptr<std::uint64_t> GetVectorTranslateMap() {return m_vectorTranslateMap;}
            ...
            ErrorCode SearchPQIndex_GPU(QueryResult& p_query, std::shared_ptr<VectorSet> vectorSet, int PQVectorsCount, int PQVectorDim, std::vector<int>& numVecPerPostinglist, std::vector<std::unique_ptr<int[]>>& postinglist, void* d_PQVectorSet, uint8_t *d_table, int* d_vectorIDs, float *d_dist, int* h_vectorIDs, float *h_dist, int totalNumVec, int threadOrder, std::unordered_set<int>& postingIDSet, SearchStats* p_stats = nullptr) const;
            ...
            float L2Distance_Dmul4(const std::uint8_t* pX, const std::uint8_t* pY, int dimVectors) const;
            float InitializeL2Distance(const std::uint8_t* pX, const std::uint8_t* pY, DimensionType m_DimPerSubvector) const;
        ...
        private:
            ...
            void LoadFusionHeadInfo(const std::string& p_file, std::vector<ListInfo>& p_listInfos);
        };
    } // namespace SPANN
} // namespace SPTAG
