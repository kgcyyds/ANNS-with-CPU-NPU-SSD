...
*/
                ...
                std::set<int>* truth, std::map<int, std::set<int>>* found)
            {
                ...
                for (uint32_t pi = 0; pi < postingListCount; ++pi)
                {
                    ...
                    request.m_callback = [&p_exWorkSpace, &queryResults, &p_index, &request, &p_stats, this](bool success)
                    {
            ...
            std::string GetPostingListVectorID(
                int postingListId,
                size_t p_postingListSize,
                Selection &p_selections,
                std::shared_ptr<VectorSet> p_fullVectors,
                ...
                const ValueType *headVector = nullptr)
            {
                ...
                size_t selectIdx = p_selections.lower_bound(postingListId);
                // std::cout<<std::endl;
                // std::cout<<"postingListId: "<<postingListId<<std::endl;
                // iterate over all the vectors in the posting list
                for (int i = 0; i < p_postingListSize; ++i)
                {
                    if (p_selections[selectIdx].node != postingListId)
            ...
            bool BuildIndex(std::shared_ptr<Helper::VectorSetReader>& p_reader, std::shared_ptr<VectorIndex> p_headIndex, Options& p_opt) {
                std::string outputFile = p_opt.m_indexDirectory + FolderSep + p_opt.m_postingListIndex;
                if (outputFile.empty())
                {
...
#pragma omp parallel for schedule(dynamic)
            ...
            bool BuildIndexFusion(std::shared_ptr<Helper::VectorSetReader>& p_reader, std::shared_ptr<VectorIndex> p_headIndex, Options& p_opt) {
                std::string outputFile = p_opt.m_indexDirectory + FolderSep + p_opt.m_fusionIndex;
                p_opt.m_replicaCount = 1;
                if (outputFile.empty())
                {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Output file can't be empty!\n");
                    return false;
                ...
                if (p_opt.m_postingPageLimit > 0)
                {
                    p_opt.m_postingPageLimit = 90 * vectorInfoSize / PageSize + 1;
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Build index with posting page limit:%d\n", p_opt.m_postingPageLimit);
                    postingSizeLimit = static_cast<int>(p_opt.m_postingPageLimit * PageSize / vectorInfoSize);
                }
                std::vector<int> vectorMapPosting(fullCount, -1);
...
#pragma omp parallel for schedule(dynamic)
                        ...
                        p_headIndex->ApproximateFusion(fullVectors, emptySet, candidateNum, selections.m_selections.data(), p_opt.m_replicaCount, numThreads, p_opt.m_gpuSSDNumTrees, p_opt.m_gpuSSDLeafSize, p_opt.m_rngFactor, p_opt.m_numGPUs, postingSizeLimit);
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Batch %d finished!\n", i);
                        ...
                        for (SizeType j = start; j < end; j++) {
                            ...
                            if (headVectorIDS.count(j) == 0) {
                                for (int resNum = 0; resNum < p_opt.m_replicaCount && selections[vecOffset + resNum].node != INT_MAX; resNum++) {
                                    ...
                                    selections[vecOffset + resNum].tonode = j;
                                    ++replicaCount[j];
                                    vectorMapPosting[j] = selections[vecOffset + resNum].node;
                                }
                            }
                        }
...
#pragma omp parallel for schedule(dynamic)
                    ...
                    OutputFusionIndexFile((i == 0) ? outputFile : outputFile + "_" + std::to_string(i),
                        p_opt.m_enableDeltaEncoding,
                        p_opt.m_enablePostingListRearrange,
                        p_opt.m_enableDataCompression,
                        p_opt.m_enableDictTraining,
        ...
        private:
                ...
                size_t p_postingListOffset)
            {
                ...
                std::uint64_t listOffset = sizeof(int) * 2;
                ...
                for (int id = 0; id < p_postingListSizes.size(); id++)
                {
                    ...
                    int postingListId = id;
                    // get posting list full content and write it at once
                    ValueType *headVector = nullptr;
                    std::string postingListFullData = GetPostingListVectorID(
                    ...
                    size_t postingListFullSize = p_postingListSizes[id] * sizeof(int);
                ...
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Final total size: %llu.\n", listOffset);
            ...
            void OutputFusionIndexFile(const std::string& p_outputFile,
                bool p_enableDeltaEncoding,
                bool p_enablePostingListRearrange,
                bool p_enableDataCompression,
                bool p_enableDictTraining,
                ...
                std::shared_ptr<VectorSet> p_fullVectors,
                size_t p_postingListOffset,
                const std::vector<int>& p_vectorMapPosting)
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Start output...\n");
                ...
                // meta size of the vector Mapping posting list
                listOffset += sizeof(int) * p_fullVectors->Count();
                ...
                for (int i = 0; i < p_fullVectors->Count(); ++i)
                {
                    int vectorMapPosting = p_vectorMapPosting[i];
                    // vectors mapping the posting list
                    if (ptr->WriteBinary(sizeof(vectorMapPosting), reinterpret_cast<char*>(&vectorMapPosting)) != sizeof(vectorMapPosting)) {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to write SSDIndex File!");
                        throw std::runtime_error("Failed to write SSDIndex File");
                    }
                }
