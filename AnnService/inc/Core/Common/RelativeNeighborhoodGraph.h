    ...
    namespace COMMON
    {
        ...
        public:
            ...
            {                
                ...
                __builtin_prefetch((const char*)nodes, 0, 3);
                __builtin_prefetch((const char*)(nodeVec), 0, 3);
                __builtin_prefetch((const char*)(insertVec), 0, 3);
                for (DimensionType i = 0; i < m_iNeighborhoodSize; i++) {
                    auto futureNode = nodes[i];
                    if (futureNode < 0) break;
                    __builtin_prefetch((const char*)(index->GetSample(futureNode)), 0, 3);
                }
