...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        public:
            ...
            virtual float InitializeL2Distance(const T* pX, const T* pY, DimensionType m_DimPerSubvector) const;
        ...
        template <typename T>
        float PQQuantizer<T>::InitializeL2Distance(const T* pX, const T* pY, DimensionType m_DimPerSubvector) const
        {
                const T* pEnd4 = pX + ((m_DimPerSubvector >> 2) << 2);
                const T* pEnd1 = pX + m_DimPerSubvector;
            ...
            for (int i = 0; i < m_NumSubvectors; i++) {
                ...
                for (int j = 0; j < m_KsPerSubvector; j++) {
                    for (int k = 0; k < m_KsPerSubvector; k++) {
                        temp_m_L2DistanceTables[m_DistIndexCalc(i, j, k)] = InitializeL2Distance(&m_codebooks[baseIdx + j * m_DimPerSubvector], &m_codebooks[baseIdx + k * m_DimPerSubvector], m_DimPerSubvector);
                    }
                }
            }
