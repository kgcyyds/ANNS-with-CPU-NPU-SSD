...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        // class BloomFilter {
        //     public:
        //         BloomFilter(size_t size, size_t numHashes): bits(size), numHashes(numHashes){}
        ...
        //         size_t hash(int item, size_t seed) const{
        //             // return std::hash<int>{}(item + seed);
        //             size_t hashNum = item + seed *31;
        //             return hashNum % bits.size();
        //         }
        // };
        ...
        class BloomFilter {
            public:
                BloomFilter(size_t size, size_t numHashes): size(size), numHashes(numHashes){
                    bitArray = new unsigned int[(size + 31) / 32];
                    std::fill(bitArray, bitArray + (size + 31) / 32, 0);  // 初始化数组元素为0
                }
                ...
                void add(int item) {
                    for(size_t i = 0; i < numHashes; i ++){
                        size_t bitPos = hash(item, i, size);
                        unsigned int mask = 1u << (bitPos % 32);
                        unsigned int* address = reinterpret_cast<unsigned int *>(bitArray) + bitPos / 32;
                        *address |= mask;
                    }
                }
            ...
            private:
                ...
                size_t hash(int item, int seed, size_t size) {
