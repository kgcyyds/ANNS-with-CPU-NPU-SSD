...
void VectorIndex::ApproximateFusion(std::shared_ptr<VectorSet>& fullVectors, std::unordered_set<SizeType>& exceptIDS, int candidateNum, Edge* selections, int replicaCount, int numThreads, int numTrees, int leafSize, float RNGFactor, int numGPUs, int postingSizeLimit)
{
    std::vector<std::thread> threads;
    threads.reserve(numThreads);
    ...
    std::vector<std::atomic_int> postingListSize(exceptIDS.size());
...
#undef DefineVectorValueType
                    ...
                    for (int i = 0; i < candidateNum && currReplicaCount < replicaCount; ++i)
                    {
                        if (queryResults[i].VID == -1 || postingListSize[queryResults[i].VID] >= postingSizeLimit)
                        {
                            break;
                        }
