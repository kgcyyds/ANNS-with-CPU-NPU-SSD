...
#define Search(CheckDeleted, CheckDuplicated, CheckFilter) \
        ...
        while (!p_space.m_NGQueue.empty()) { \
            ...
            __builtin_prefetch((const char *)node, 0, 3); \
            for (DimensionType i = 0; i <= checkPos; i++) { \
                __builtin_prefetch((const char *)(m_pSamples)[node[i]], 0, 3); \
            } \
...
*/
        ...
        void Index<T>::Search(COMMON::QueryResultSet<T>& p_query, COMMON::WorkSpace& p_space, std::function<bool(const ByteArray&)> filterFunc) const
        {
            ...
            while (!p_space.m_NGQueue.empty()) {
                NodeDistPair gnode = p_space.m_NGQueue.pop();
                SizeType tmpNode = gnode.node;
                const SizeType* node = m_pGraph[tmpNode];//m_pGraph[tmpNode] 代表着与节点 tmpNode相邻的节点
                __builtin_prefetch((const char*)node, 0, 3);//__builtin_prefetch 函数，该函数是一个内置的指令级别优化函数，用于将数据预取到高速缓存中，以提高后续访问该数据的速度
                for (DimensionType i = 0; i <= checkPos; i++) {
                    auto futureNode = node[i];
                    if (futureNode < 0) break;
                    __builtin_prefetch((const char*)(m_pSamples)[futureNode], 0, 3);
                }
                if (gnode.distance <= p_query.worstDist()) 
                {
                    SizeType checkNode = node[checkPos];
                    if (checkNode < -1)//checkNode = -1 或者为 head节点
                    {
                        const COMMON::BKTNode& tnode = m_pTrees[-2 - checkNode];
                        SizeType i = -tnode.childStart;
                        do
        ...
        template<typename T>
        void Index<T>::SearchIndex(COMMON::QueryResultSet<T>& p_query, COMMON::WorkSpace& p_space) const
        {
            std::shared_lock<std::shared_timed_mutex> lock(*(m_pTrees.m_lock));
            m_pTrees.InitSearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space);
            m_pTrees.SearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space, m_iNumberOfInitialDynamicPivots);
            ...
            while (!p_space.m_NGQueue.empty()) {
                ...
                // gnode.distance = m_fComputeDistance(p_query.GetQuantizedTarget(), (m_pSamples)[tmpNode], GetFeatureDim());
                if (gnode.distance <= p_query.worstDist()) 
                {
                    SizeType checkNode = node[checkPos];
                    p_query.AddPoint(tmpNode, gnode.distance);
        ...
        ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
        {
            ...
            if (!workSpace) {
                ...
                workSpace->Initialize(max(m_iMaxCheck, m_pGraph.m_iMaxCheckForRefineGraph), m_iHashTableExp - 3);//m_iMaxCheck: 1024, m_pGraph.m_iMaxCheckForRefineGraph: 16324, m_iHashTableExp: 4
            }

            workSpace->Reset(m_iMaxCheck, p_query.GetResultNum());// p_query.GetResultNum(): InternalResultNum
        ...
        /*
            tyh: MemoryIndex Search with PQ vector
        */
        // template <typename T>
        ...
        // template<typename T>
        // void Index<T>::SearchIndexWithGPU(COMMON::QueryResultSet<T>& p_query, std::shared_ptr<VectorSet> PQvectorSet, COMMON::WorkSpace& p_space, std::shared_ptr<std::uint64_t> p_vectorTranslateMap) const
        // {
        //     std::shared_lock<std::shared_timed_mutex> lock(*(m_pTrees.m_lock));
        //     m_pTrees.InitSearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space);
        //     m_pTrees.SearchTrees(m_pSamples, m_fComputeDistance, p_query, p_space, m_iNumberOfInitialDynamicPivots);
        ...
        //             if (gnode.distance > p_space.m_Results.worst() || p_space.m_iNumberOfCheckedLeaves > p_space.m_iMaxCheck) 
        //             {
        //                 //std::cout<<"m_iNumberOfCheckedLeaves: "<<p_space.m_iNumberOfCheckedLeaves<<std::endl;//5163
        //                 p_query.SortResult();
        //                 return;
        //             }
        //         }
        ...
        //             //使用PQ向量
        //             SizeType global_index = static_cast<SizeType>((p_vectorTranslateMap.get())[nn_index]);
        //             float distance2leaf = L2Distance(reinterpret_cast<const uint8_t *>(p_query.GetnewPQTarget()), reinterpret_cast<const uint8_t *>(PQvectorSet->GetVector(global_index)), GetFeatureDim());
                    
        //             // float distance2leaf = L2Distance(reinterpret_cast<const uint8_t *>(p_query.GetheadPQTarget()), reinterpret_cast<const uint8_t *>(PQvectorSet->GetVector(nn_index)), GetFeatureDim());
        ...
        //     const DimensionType checkPos = m_pGraph.m_iNeighborhoodSize - 1;
        //     int L = 300;
        //     NeighborPriorityQueue CandidateSet(L);

        //     NodeDistPair gnode = p_space.m_NGQueue.pop();
        //     // SizeType start_index = static_cast<SizeType>((p_vectorTranslateMap.get())[gnode.node]);
        //     // float begindist = L2Distance(reinterpret_cast<const uint8_t *>(p_query.GetnewPQTarget()), reinterpret_cast<const uint8_t *>(PQvectorSet->GetVector(start_index)), GetFeatureDim());
        //     Neighbor StartPoint(gnode.node, gnode.distance);
        //     CandidateSet.insert(StartPoint);
        //     while (CandidateSet.has_unexpanded_node()) {
        //         auto nbr = CandidateSet.closest_unexpanded();
        //         nbr.distance = m_fComputeDistance(p_query.GetQuantizedTarget(), (m_pSamples)[nbr.id], GetFeatureDim());
        //         p_query.AddPoint(nbr.id, nbr.distance);

        //         //get neighbors
        //         const SizeType* node = m_pGraph[nbr.id];
        //         __builtin_prefetch((const char*)node, 0, 3);//__builtin_prefetch 函数，该函数是一个内置的指令级别优化函数，用于将数据预取到高速缓存中，以提高后续访问该数据的速度
        //         for (DimensionType i = 0; i <= checkPos; i++) {
        //             auto futureNode = node[i];
        //             if (futureNode < 0) break;
        //             __builtin_prefetch((const char*)(m_pSamples)[futureNode], 0, 3);
        //         }
        //         //compute neighbor PQ dist and insert
        //         for (DimensionType i = 0; i <= checkPos; i++) 
        //         {
        //             SizeType nn_index = node[i];
        //             if (nn_index < 0) break;
        ...
        //             float distance2leaf = L2Distance(reinterpret_cast<const uint8_t *>(p_query.GetnewPQTarget()), reinterpret_cast<const uint8_t *>(PQvectorSet->GetVector(global_index)), GetFeatureDim());
        //             //float distance2leaf = m_fComputeDistance(p_query.GetQuantizedTarget(), (m_pSamples)[nn_index], GetFeatureDim());
        //             // float distance2leaf = m_fComputeDistance(reinterpret_cast<const T *>(p_query.GetQuantizedTarget()), reinterpret_cast<const T *>(PQvectorSet->GetVector(global_index)), GetFeatureDim());
        //             Neighbor nn(nn_index,distance2leaf);
        //             CandidateSet.insert(nn);
        //         }
        //     }
        //     p_query.SortResult();
        // }
        ...
        // template<typename T>
        // ErrorCode Index<T>::SearchIndexWithGPU(QueryResult &p_query, std::shared_ptr<VectorSet> PQvectorSet, std::shared_ptr<std::uint64_t> p_vectorTranslateMap, bool p_searchDeleted) const
        // {
        //     if (!m_bReady) return ErrorCode::EmptyIndex;
        ...
        //     if (!workSpace) {
        //         workSpace.reset(new COMMON::WorkSpace());
        //         workSpace->Initialize(max(m_iMaxCheck, m_pGraph.m_iMaxCheckForRefineGraph), m_iHashTableExp);//m_iMaxCheck: 1024, m_pGraph.m_iMaxCheckForRefineGraph: 16324, m_iHashTableExp: 4
        //     }

        //     workSpace->Reset(m_iMaxCheck, p_query.GetResultNum());// p_query.GetResultNum(): InternalResultNum

        //     //SearchIndexWithGPU(*((COMMON::QueryResultSet<T>*)&p_query), *workSpace, p_vectorTranslateMap, p_searchDeleted, true);//p_searchDeleted false
        //     SearchIndexWithGPU(*((COMMON::QueryResultSet<T>*)&p_query), PQvectorSet, *workSpace, p_vectorTranslateMap);
